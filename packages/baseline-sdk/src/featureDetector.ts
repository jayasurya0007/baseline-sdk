...
export class FeatureDetector {
  constructor(private dataSource: BaselineDataSource) {}

  public detectFeatures(source: string, target: BaselineLevel): DetectedFeature[] {
    const issues: DetectedFeature[] = [];
  ...
  private isSupported(featureBaseline: BaselineLevel, target: BaselineLevel): boolean {
    const order = ['limited', 'newly', 'widely'];
    return order.indexOf(featureBaseline) >= order.indexOf(target);
  }
  
  private getLineAndColumn(source: string, index: number): { line: number; column: number } {
